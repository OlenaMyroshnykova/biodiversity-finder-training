...
def normalize_column_names(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df.rename(columns={
        'scientificName':'scientific_name','acceptedScientificName':'accepted_scientific_name','countryCode':'country_code',
        'decimalLatitude':'decimal_latitude','decimalLongitude':'decimal_longitude','basisOfRecord':'basis_of_record',
        'individualCount':'individual_count','coordinateUncertaintyInMeters':'coordinate_uncertainty_meters','class':'taxon_class','order':'taxon_order'
    })

def keep_required_data(df: pd.DataFrame) -> pd.DataFrame:
    """Elimina filas sin campos básicos."""
    required_columns = ['key','scientific_name','kingdom','phylum','taxon_class','family','country_code','decimal_latitude','decimal_longitude','year','month','basis_of_record']
    existing_required_columns = [column for column in required_columns if column in df.columns]
    return df.dropna(subset=existing_required_columns).copy()
...
def clean_coordinates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return result_df[result_df['decimal_latitude'].between(-90,90) & result_df['decimal_longitude'].between(-180,180)].copy()
...
def clean_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result_df = result_df[result_df['year'].between(1900,2100) & result_df['month'].between(1,12)].copy()
...
def remove_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    subset_columns = ['scientific_name','decimal_latitude','decimal_longitude','year','month','basis_of_record']
