...
La descarga ya no está anclada a especies bonitas de demo. El plan usa
muestreo taxonómico neutral: clases, reinos y grupos amplios. Las etiquetas
semánticas de búsqueda se crean después, en ``search_tags.py``.
"""
...
def build_global_query_plan(country: str | None = "GLOBAL") -> list[GBIFQuery]:
    """Construye un plan neutral por grupos taxonómicos amplios.

    Si ``country`` no es GLOBAL, se añade filtro de país a todas las consultas.
    No se usan nombres de especies concretas ni descripciones tipo demo.
    """
    ...
    taxon_specs = [
        ("class_mammalia", "Clase Mammalia", "Mammalia", "CLASS", 0.13),
        ("class_aves", "Clase Aves", "Aves", "CLASS", 0.13),
        ("class_reptilia", "Clase Reptilia", "Reptilia", "CLASS", 0.11),
        ("class_amphibia", "Clase Amphibia", "Amphibia", "CLASS", 0.10),
        ("class_insecta", "Clase Insecta", "Insecta", "CLASS", 0.13),
        (
            "class_actinopterygii",
        ...
        ("kingdom_plantae", "Reino Plantae", "Plantae", "KINGDOM", 0.10),
        ("kingdom_fungi", "Reino Fungi", "Fungi", "KINGDOM", 0.06),
    ]
...
def download_biodiversity_training_dataset(
    ...
    """Descarga un dataset global por grupos taxonómicos amplios."""
    ...
    for query in query_plan:
        ...
        print(
            f"- {query.source_query}: objetivo {query_records:,} registros | "
            f"{query.description}",
            flush=True,
        )
